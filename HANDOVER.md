...
### Sprint J — World-class extensibility 2026-04-29 (no secrets) 🟡 IN PROGRESS

Goal stated by the owner verbatim: *"عاوز السستم عالمي قابل للتوسع من الاوبشنز بتاعتو من غير ما اغير انا في الكود بعد كدا"* — make the system globally extensible from its admin UI without code changes. Adding a new country must auto-link its currency, let admins set manager email + cutoff + which brands operate there in a single dialog, and have the new country immediately surface in cases, promo, reports, and filters.

- ✅ **Atomic country activation flow** — `/admin/countries` now ships a single dialog (manager email, daily cutoff, sort order, brand checkboxes) that creates-or-updates the `Country` row AND reconciles the `BrandCountry` m2m in one `prisma.$transaction`. Audit log records before/after `brandIds` so deactivations are traceable. The bare toggle is gone; admins can no longer activate a country without committing to who manages it and which brands operate there. (commit `84793ce`)
- ✅ **Enforce BrandCountry on case creation** — `createCaseAction` previously had a non-blocking TODO. It now hard-checks (a) country is active, (b) brand exists + isActive, (c) `BrandCountry { brandId, countryId, isActive: true }` row exists. Errors point the user back to `/admin/countries → Edit` for resolution rather than silently allowing mismatched data. (commit `c500d9e`)
- ✅ **Enforce BrandCountry on promo config creation** — `createPromoConfigAction` now mirrors the case-creation guard so `PromoConfig` rows can only exist for brand-country pairs the admin has wired up. Same actionable error pointing back to the country editor. (commit `8fadbb0`)
- ✅ **Drop /admin/currencies and group admin nav** — currency is owned by `CountryRegistry.currencyId` and surfaces inside the country activation dialog, so a separate Currencies admin page was dead weight. Page directory deleted, sidebar entry removed, `nav.currencies` + `admin.currencies` translation blocks pruned, and the unused `Coins` icon import dropped. The flat 22-item Admin sidebar section is now six labeled sub-sections — *People & Access*, *Geography & Brands*, *Workflow Rules*, *Communication*, *Schedules & Reports*, *System* — so related controls cluster visually instead of fighting for space in one big list. `adminOnly` + per-module disable filters keep working unchanged. (commit `940c2e2`)
- ✅ **`/admin/settings` is now the admin hub; sidebar trimmed to daily-use items** — the admin sidebar section was still 22 items wide. Now it is **two**: *Pending Approvals* (admins act on this daily) and *Settings*. Everything else (users, brands, branches, payment methods, root causes, SLA rules, fraud signals, automation rules, email templates, store templates, email log, batch schedules, scheduled reports, cron status, audit log, backup, modules, design tokens, system info) lives inside `/admin/settings`, rendered as six titled card grids with an icon + label + one-line description per surface. The previous workspace key/value configuration (SLA thresholds, company info, feature flags) is preserved at the bottom of the same page under a *Workspace configuration* heading so admins do not bounce between routes. (commit `2969787`)
- ✅ **Refund Pool redesign + 5-country demo scenario** — owner asked for a single canvas that makes "what's approved and waiting on me" obvious. The bare 3-tab Approvals/KNET/Aura layout never showed an end-to-end case, so a new default `Pool` tab was added on `/operations` (commit `3aaa66b`):
  - Master-detail layout: left search + filter chips by payment rail and country, scrollable case cards (country flag, case #, customer, brand, order #, payment, amount, status). Center: full selected case (refund vs order amount, approval batch + approver, customer contact buttons `tel:` / `wa.me` / `mailto`, refund components with ARN status + batch link or "Not yet batched", Aura sidecar, customer contact log, manager approval reply, full activity timeline). Right: live KNET / Aura batch status with ARN progress, plus a global recent-activity feed.
  - Existing Approvals / KNET / Aura tabs stay so the batch workflow is unchanged. The Pool tab is the new default.
  - Opt-in seeder `seedRefundPoolScenario()` (gated on `SEED_REFUND_POOL=1`) produces 5 realistic APPROVED cases (KW Starbucks KNET 8.75 KWD, SA Chipotle Mastercard 95 SAR, AE Starbucks Apple Pay 35 AED, BH Chipotle Mastercard 12 BHD + 200 Aura, OM Starbucks KNET 6 OMR) — each with its own ApprovalBatch (manager email + reply text), 1-2 contact attempts logged via ActivityLog with channel metadata + mirrored CaseNotes, and a 5-event timeline. Run with `cd packages/db && DATABASE_URL="file:./dev.db" SEED_REFUND_POOL=1 pnpm prisma db seed`.
- ✅ **Role clarity + naming pass** — owner asked for clearer role boundaries and a less ambiguous label for the user-signup queue. Done in one commit (`ca90135`) without adding new roles since the seeded `Role` table already has every key needed:
  - **ADMIN** — full access to settings, hub, and operations (unchanged).
  - **AGENT** — Cases + Stores Communication + Promo (use, not pool admin); cannot reach Refund Pool.
  - **REFUND_AGENT** — AGENT capabilities + Refund Pool. Was previously locked out: `/operations` and the sidebar both checked for a role string `'OPERATIONS'` that **isn't seeded anywhere** (the seeded key is `REFUND_AGENT`). The guard now accepts `REFUND_AGENT` (with `'OPERATIONS'` kept as a legacy alias) so a refund agent can actually reach the page that exists for them.
  - "Pending Approvals" → "User Requests" (sidebar + page H1 + i18n EN/AR + settings hub card). New subtitle makes the purpose explicit: *new signup requests waiting for an admin to assign a role and primary country before they can sign in*.
  - "Refund Operations" → "Refund Pool" (sidebar + page H1 + i18n EN/AR). Page now opens with one paragraph explaining the pool, plus a one-line description above each tab (Approvals / KNET / Aura) so the workflow is self-documenting.
- 🟡 **Push to GitHub blocked** — git-manager proxy is returning `403 Forbidden` on every push attempt for this branch (fetch + ls-remote work; only push fails). All extensibility + role-clarity commits live locally on `devin/1777249813-continue-roadmap` waiting for the GitHub integration to be re-authorized at https://app.devin.ai/settings/integrations or org settings. Once auth is restored: `git push origin devin/1777249813-continue-roadmap`.
- ⬜ **PII encryption Prisma `$extends` wiring** — deliberately deferred. `lib/crypto/pii.ts` is ready and key-gated, but wiring it into a `query` + `result` extension over `RefundCase.{customerEmail,customerPhone,customerName}` would silently break `where: { customerEmail }` exact-match lookups (AES-GCM produces non-deterministic ciphertext). Needs an explicit rollout plan: deterministic encryption / blind index column / accept that lookup-by-PII becomes a scan. Punted until the owner picks a tradeoff.
